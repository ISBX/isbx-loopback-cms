  ...
  return {
    ...
    link: function(scope, element, attrs) {
      ...
      scope.onFileSelect = function($files) {
        ...
        loadImage.parseMetaData(selectedFile, function(data) {
          var options = { canvas: true };
          if (data.exif) {
            options.orientation = data.exif.get('Orientation');
            scope.orientation = options.orientation;
          }
          var loadingImage = loadImage(selectedFile, function(img) {
            ...
            if (scope.options && scope.options.urlKey) { //When field options involve a reference table use model key and urlKey as reference
            ...
            } else { //No table reference (file URL assigned directly into current model's field)
              scope.modelData.__ModelFieldImageData[scope.key] = imageData;
            }

            //Set the preview image via scope.imageUrl binding
            scope.imageUrl = img.toDataURL();
            ...
            if (scope.options && scope.options.export) {
              ...
              scope.exportImages(img, function() {
                scope.uploadStatus = "Upload File";
                scope.$apply();
              });
            } else if (scope.options && scope.options.resize) {
              scope.resizeImage(img, scope.options.resize, function(blob) {
      ...
      scope.exportImages = function(image, callback) {
        ...
        scope.resizeImage(image, settings, function(blob) {
          ...
          index++;
          scope.exportImages(image, callback, index);
        });
      };

      scope.resizeImage = function(img, settings, callback) {
        if (typeof settings == 'undefined') settings = {};
        var options = {canvas:true};
        if(scope.orientation) options.orientation = scope.orientation;
        if(settings.width) options.maxWidth = settings.width;
        if(settings.height) options.maxHeight = settings.height;
        if (!settings.aspect) settings.aspect = 'fit';
        switch(settings.aspect) {
          case "stretch":
            options.sourceWidth = settings.width;
            options.sourceHeight = settings.height;
            break;
          case "fill":
            options.cover = true;
            break;
        ...
        loadImage(img, function(scaledImage) {
          var dataUrl = scaledImage.toDataURL();
