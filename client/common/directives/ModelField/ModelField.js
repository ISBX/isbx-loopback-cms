  ...
  function getTemplate(type, scope) {
    ...
    switch(type) {
      ...
      case 'number-decimal':
        template = '<label class="col-sm-2 control-label">{{ display.label || key }}:</label>\
          <div class="col-sm-10">\
            <input type="number" min="{{ display.minValue }}" max="{{ display.maxValue }}" decimal-scale="{{ display.scaleValue }}" ng-model="data[key]" ng-pattern="{{ display.pattern }}" ng-disabled="{{ display.readonly }}" ng-required="{{ model.properties[key].required }}" class="field form-control">\
            <div class="model-field-description" ng-if="display.description">{{ display.description }}</div>\
          </div>';
        break;
      case 'phoneNumber':
